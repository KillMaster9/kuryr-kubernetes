...
def create(resp=None, body=None, auth_token=None):
    ...
    if 'token' in body:
        ...
        return IamAccessInfo(body, auth_token)
...
class BaseIamAccessInfo(AccessInfo):
    """An object encapsulating base IAM auth token from identity service.
    ...
    IAM Access Info:
    {
        ...
        "token_type":"Bearer",
        "not-before-policy":0,
        "session_state":"84cfbf17-5809-4971-84a0-c9bc817ba47c",
        "scope":"profile email incloudos-scope"
    }
    """

    _service_catalog_class = None

    def __init__(self, body, auth_token=None, **kwargs):
        if not auth_token:
            auth_token = body['access_token']
        super(BaseIamAccessInfo, self).__init__(body, auth_token)
        self._issued = None
        self._expires = None
        self._header = None
        self._payload = None
    ...
    def parse_access_token(self):
        ...
        header
        {
            "alg": "RS256",
            "typ": "JWT",
            "kid": "h1OGJwSszA8HcHEAhz-RWWifpMflZ8-5a4qO05tqvBo"
        }
        payload
        {
            "exp": 1703129586,
            "iat": 1703125986,
            "jti": "818b0bd1-8ee7-4ec1-a38c-d3e6dd989f23",
            "iss": "http://10.238.28.75:8080/auth/realms/master",
            "sub": "130582cb-14df-47de-b192-cf92d0cbfc63",
            "typ": "Bearer",
            "azp": "admin-cli",
            "session_state": "7a52424c-a616-46a8-bd41-f9c7c748e9b7",
            "acr": "1",
            "scope": "profile email incloudos-scope",
            "email_verified": false,
            "name": "TmytcJ9S",
            "preferred_username": "TmytcJ9S"
        }
        '''

        token_splits = self.access_token.split('.')
        self._header = json.loads(utils.base64url_decode(token_splits[0]))
        self._payload = json.loads(utils.base64url_decode(token_splits[1]))
        self._hmac = utils.base64url_decode(token_splits[2])

        self._issued = datetime.datetime.utcfromtimestamp(self._payload['iat'])
        self._expires = datetime.datetime.utcfromtimestamp(self._payload['exp'])
...
class IamAccessInfo(BaseIamAccessInfo):
    ...
    @property
    def refresh_expires(self):
        return self._data['refresh_expires_in']
    ...
    @property
    def not_before_policy(self):
        return self._data['not-before-policy']
